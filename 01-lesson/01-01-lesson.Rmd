...
essential for getting started with data analysis.
...
Before working with more data objects, pause to classify variables by
the kind of information they contain.

- **Numerical** variables measure or count quantities.
- **Categorical** variables place observations into groups.
- **Ordinal** variables are categorical variables with a meaningful
  order.
- **Identifier** variables label observations but should not usually be
  summarized as measurements.

For each variable below, write whether it is numerical, categorical,
ordinal, or an identifier.

```{r variable-type-checkpoint, exercise=TRUE}
# student_id:
# final_exam_score:
# class_year, such as first-year, sophomore, junior, senior:
# favorite_campus_event:
```
...
In one or two sentences, explain the difference between assigning a
value and printing a value in R.

```{r assignment-printing-reflection, exercise=TRUE}
